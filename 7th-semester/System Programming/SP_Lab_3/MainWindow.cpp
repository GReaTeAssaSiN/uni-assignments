...
//Обработка при изменении ячеек МППФ.
void MainWindow::on_tableWidget_OMH_cellChanged(int row, int column)
{
    manager->UpdateRowInSpecifiedTable(ui->tableWidget_OMH, row, column);
}
...
void MainWindow::on_pushButton_first_clicked()
{
    ...
    if(!this->PP.FirstPass(this->type_addr, source_code, opcode_table, ui->tableWidget_auxTable, sup_table, ui->tableWidget_ToSN,
        ...
        //Изменение размера ячеек таблиц.
        ui->tableWidget_auxTable->resizeColumnsToContents(); //Вспомогательная таблица.
        //Визуализация кнопок.
...
void MainWindow::on_pushButton_second_clicked()
{
    ...
    //Изменение размера ячеек таблиц.
    ui->tableWidget_modTable->resizeColumnsToContents();//Таблица модификаторов.
    //Визуализация кнопок.
    ui->pushButton_first->setEnabled(true);
    ui->pushButton_second->setEnabled(false);
}
