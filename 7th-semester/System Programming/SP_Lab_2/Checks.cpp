...
bool Checks::CheckRowSourceCode(QTextEdit *textEdit_FPE, const int &row, const QString& prog_name, const TableCodeOperation& TCO, const QString &label, const QString &mnemonic_code)
{
    ...
    if (!label.isEmpty()){//Если метка не пустая.
        ...
        if (row != 0 && label == prog_name.toUpper()){
            error = "Строка " + QString::number(row+1) + ": Символическое имя совпадает с названием программы без учета регистра. Измените название программы или символическое имя!\n";
            textEdit_FPE->append(error);
            return false;
        }
    }
