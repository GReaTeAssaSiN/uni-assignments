                ...
                if (current_mnemonic_code != ""){
                    if (current_mnemonic_code == "START"){
                        ...
                        if (i == 0 && !start_flag){
                            ...
                            if (operand1.isEmpty()){
                                this->load_address = 0;
                                textEdit_FPE->append("ПРЕДУПРЕЖДЕНИЕ! Строка " + QString::number(i + 1) + ": АЗ программы установлен в 0. Рассматривается программа в перемещаемом формате.\n");
                            }
                            else{
                            ...
                            if (!label_flag){
                                ...
                                if (operand1.isEmpty()){
                                    ...
                                    if (!checks.CheckAmountMemoryForAddress(operand1)){
                                        ...
                                        this->address_entry_point = Convert::ConvertHexToDec(operand1);
                                    }
                                }
                               ...
                               const std::vector<SupportTable> &sup_table, const SymbolicNameTable &symbolic_table)
{
    ...
    //Очищаем таблицу модификаторов и устанавливаем 1 столбец и 1 строку.
    tableWidget_modTable->clear();
    tableWidget_modTable->setColumnCount(1);
    ...
    for (int i{1}; i < sup_table.size(); i++){
        ...
        if (checks.CheckDirective(sup_table[i].operation_code) != ""){//Директивы BYTE, WORD, RESB, RESW.
            ...
            int type_addr{Convert::ConvertHexToDec(sup_table[i].operation_code) & 0x03};
