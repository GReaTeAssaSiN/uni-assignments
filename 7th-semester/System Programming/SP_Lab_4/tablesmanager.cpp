...
void TablesManager::LoadOneLineToSymbolicNamesTable(QTableWidget *symb_table, const SymbolicNamesTable &symb, QString name)
{
    ...
    else
    {
        ...
        symb_table->setItem(last_row, 2, new QTableWidgetItem(""));
    }
}

void TablesManager::LoadElemOfListToSymbolicNamesTable(QTableWidget *symb_table, const OneSymbolicName &symb)
{
    if (symb_table == nullptr)
    {
        qDebug() << "Pointer to symb_table was empty in TablesManager::LoadElemOfListToSymbolicNamesTable.";
        return;
    }
    else
    {
        int last_row = symb_table->rowCount() - 1;
        symb_table->setItem(last_row, 0, new QTableWidgetItem(symb.name));
        symb_table->setItem(last_row, 1, new QTableWidgetItem(""));
        symb_table->setItem(last_row, 2, new QTableWidgetItem(symb.address));
    }
}

//Функции загрузок примеров.
...
std::vector<AssemblerInstruction> TablesManager::ParseAssemblerSourceCode(QTextEdit *source_text_TextEdit)
{
    ...
    for (const QString &line : lines)
    {
        ...
        {
            // Если да, то присваиваем элементы с учетом смещения.
...
void TablesManager::ExportFormattedSourceAssemblerCodeToField(std::vector<AssemblerInstruction> &source_code, QTextEdit* textEdit_source)
{
    textEdit_source->clear();
    for (const auto& instruction: source_code){
        if (instruction.label->isEmpty() && instruction.mnemonic.isEmpty() && instruction.operand1.isEmpty() && instruction.operand2->isEmpty())
            continue;
        LoadOneLineToAssemblerSourceCode(textEdit_source, instruction);
    }
}
