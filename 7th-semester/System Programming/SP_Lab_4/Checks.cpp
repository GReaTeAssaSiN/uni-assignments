...
bool Checks::CorrectTableCodeOperation(const CodeOperationTable& opCodeTable, QTextEdit* fpe_text)
{
    ...
    for (size_t row{}; row < opCodeTable.getOpCodeTableSize() - 1; ++row)
    {
        ...
        {
            fpe_text->append("Пустая ячейка в ТКО не допустима.\n");
            return false;
        }
        if (opCode_Table[row].mnemonic_code.isEmpty() && (!opCode_Table[row].binary_code.isEmpty() || !opCode_Table[row].size.isEmpty()))
        {
        ...
        if (!opCode_Table[row].mnemonic_code.isEmpty())
        {
            ...
            {
                fpe_text->append("МКОП в ТКО должен быть от 1 до 6 символов, двоичный код МКОП - от 1 до 2 символов, длина МКОП в байтах - не более одного символа.\n");
                return false;
            }
            //Проверка посимвольно МКОП.
            if (!CheckCharsMCOP(opCode_Table[row].mnemonic_code))
            {
                fpe_text->append("МКОП в ТКО должен начинаться с буквы и не содержать некорректных символов, отличных от цифр и букв латинского алфавита: " + opCode_Table[row].mnemonic_code +".\n");
                return false;
            }
            //Проверка МКОП на зарезервированное слово.
            if (!CheckIncorrectName(opCode_Table[row].mnemonic_code))
            {
                fpe_text->append("МКОП в ТКО не может совпадать с директивой или регистром: " + opCode_Table[row].mnemonic_code +".\n");
                return false;
            }
            ...
            if (CheckAmountMemoryForAddress(opCode_Table[row].binary_code)){
                ...
                if (dec_BinCode == 0)
                {
                    fpe_text->append("Двоичный код МКОП в 16-ричной СИ в ТКО не может быть равен 0: " + opCode_Table[row].binary_code +".\n");
                    return false;
                }
                if (dec_BinCode > MAX_BINARY_CODE)
                {
                    fpe_text->append("Двоичный код МКОП в 16-ричной СИ в ТКО не должен превышать " + Convert::DecToHex(MAX_BINARY_CODE) + ": " + opCode_Table[row].binary_code +".\n");
                    return false;
                }
            }
            else
            {
                fpe_text->append("Двоичный код МКОП в ТКО должен быть представлен из символов 16-ричной СИ: " + opCode_Table[row].binary_code +".\n");
                return false;
            }
            //Проверка длины команды МКОП в байтах.
            if (!CheckCorrectAmountMemoryForCodeOperation(opCode_Table[row].size))
            {
                fpe_text->append("Допустимы команды от 1 до 4 байт (указываются в 10-чной СИ): " + opCode_Table[row].size + ".\n");
                return false;
            }
            ...
            for (size_t k = row + 1; k < opCodeTable.getOpCodeTableSize() - 1; ++k)
            {
                ...
                {
                    fpe_text->append("Найдены совпадения в двоичных кодах МКОП!\nСовпадающий двоичный код МКОП: " + first_binCode + ".\n");
                    return false;
                }
            }
        }
...
//Проверка ТСИ на корректность.
bool Checks::CheckToSNForCorrectAddress(QTextEdit *textEdit_Errors, QTableWidget *tableWidget_ToSN)
{
    int rowCount = tableWidget_ToSN->rowCount();
    for (int i{}; i < rowCount; ++i){
        QTableWidgetItem *Item1 = tableWidget_ToSN->item(i, 0); // 1-й столбец
        QTableWidgetItem *Item2 = tableWidget_ToSN->item(i, 1); // 2-й столбец.
        // Проверяем, что элемент существует и его значение не пустое.
        if (Item1 && Item2 && Item2->text().isEmpty()) {
            textEdit_Errors->append("Найдено неопределенное символическое имя: " + tableWidget_ToSN->item(i, 0)->text() + "!\n");
            return false;
        }
    }
    return true;
...
bool Checks::CheckSymbolicNameInOperandPart(QString operand, QTextEdit* textEdit_Errors, const int& row)
{
    ...
    {
        textEdit_Errors->append("Строка " + QString::number(row + 1) + ": Длина СИ должна быть не более 6 символов!\n");
        return false;
    }
    //Символы метки.
    if (row != 0 && !CheckCharsSymbolicName(operand))
    {
